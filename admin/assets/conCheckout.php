...
$ans = $_POST['ans'];
$cid = $_POST['cid'];
$tid = preg_replace('/[^a-zA-Z0-9]/', '', $_POST['tid']);
...
if($ans =='Yes'){
    ...
    $query = mysqli_query($db_conx, $sql);
    if(!$query){
        echo "Could not update saled to confirmed";
        exit();
    }
    if($query){
        ...
        <h1>Thank You '.$name.'!<br/><small>Your Purchase Order has been confirmed</small></h1>
            ...
            <table style="text-align:center; padding:10px; width:100%; color:grey;">
                ...
                for($i=0;$i<count($itemNoArr);$i++){
                    $itemNo = $itemNoArr[$i];
                    $sql = "SELECT descr, price, img1 FROM items WHERE itemNo='$itemNo' AND ownerID='$cid' LIMIT 1";
                    $query = mysqli_query($db_conx, $sql);
                    if(!$query){
            ...
            if(mail($email,$subject,$message,$headers)){
                echo 'Payment confirmed successfully. Customer has been emailed';
                exit();
            }
        }
...
if($ans =='No'){
    ...
    if(mail($email,$subject,$message,$headers)){
        ...
        $cQuery = mysqli_query($db_conx, $cSql);
        echo 'Payment has been discredited successfully. Customer has been emailed';;
        exit();
    }
